...
function exportPhrases() {
  loadPhrases(function(phrases) {
    ...
    const data = 'data:text/plain;charset=utf-8,' + encodeURIComponent(phrasesStr);
...
function clearPhrases() {
  if (confirm("Are you sure you want to clear the word list? This cannot be undone.")) {
    initPhrases();
  }
}

document.getElementById("fileElem").addEventListener("change", handleFiles);
document.getElementById("exportButton").addEventListener("click", exportPhrases);
document.getElementById("importButton").addEventListener("click", importPhrases);
